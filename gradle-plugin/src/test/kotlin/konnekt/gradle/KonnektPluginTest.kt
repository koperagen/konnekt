...
class KonnektPluginTest : FreeSpec({
  ...
  "project without plugin failes" {
    shouldThrowExactly<UnexpectedBuildFailure> {
      val result = GradleRunner {
        defaultConfig()
        val src = projectDir.resolve("src").let {
          it.mkdir()
      ...
      result.asClue {
        it.task(":compileKotlin")?.outcome shouldBe TaskOutcome.FAILED
      }
    }
  }
})
